...
def Find_File_in_UserDir_with_Version_Dir(UserDir, VersionDir, SearchName):
    ...
    VerDir = X02.Dir(SeachDir + VersionDir, vbDirectory)
    if VerDir == '':
    ...
    while 1:
        ...
        if not (True):
            break
    if X02.Dir(SeachDir + VerDir + SearchName) == '':
...
def Create_Set_Fuses_Cmd_file(ResultName, Mode, DstDir):
    ...
    VBFiles.writeText(fp, 'REM This file was generated by \'' + PG.ThisWorkbook.Name + '\'  ' + X02.Time, '\n')
...
def Write_Fuses(ComPort, Mode, DstDir):
    ...
    if CommandStr != '':
        ...
        _select58 = Res
        if (_select58 == Success) or (_select58 == Timeout):
            # No additional error message. They have been shown in the DOS box
            pass
        else:
            X02.MsgBox(M09.Get_Language_Str('Fehler ') + Res + M09.Get_Language_Str(' beim starten des Arduino Programms \'') + CommandStr + '\'', vbCritical, M09.Get_Language_Str('Fehler beim starten des Arduino programms'))
        if X02.Dir(ResFile) != '':
...
def Compile_and_Upload(SrcDir, InoName, CommandStr, ResFile):
    ...
    # No timeout to be able to study the results in case of an error
    _select59 = Res
    if (_select59 == Success) or (_select59 == Timeout):
        # No additional error message. They have been shown in the DOS box
        pass
    else:
        X02.MsgBox(M09.Get_Language_Str('Fehler ') + Res + M09.Get_Language_Str(' beim starten des Arduino Programms \'') + CommandStr + '\'', vbCritical, M09.Get_Language_Str('Fehler beim starten des Arduino programms'))
    if X02.Dir(ResFile) != '':
        ...
        Debug.Print('Compile and upload duaration: ' + X02.Format(X02.Time - Start, 'hh:mm:ss'))
...
def Get_COMPortStr(ComPortColumn):
    ...
    _fn_return_value = '\\\\.\\COM' + M07a.ComPortPage().Cells(M01.SH_VARS_ROW, ComPortColumn)
    return _fn_return_value
...
def Check_If_Arduino_could_be_programmed_PatGen():
    ...
    _fn_return_value ,BuildOptions = M07b.Check_If_Arduino_could_be_programmed_and_set_Board_type(M01.COMPrtT_COL, M01.BuildOT_COL, BuildOptions)
...
def Create_Inp_Prtx(DstDir, Correct_Pins):
    ...
    # VB2PY (UntranslatedCode) On Error GoTo WriteError
    VBFiles.openFile(fp, Name, 'w') 
    VBFiles.writeText(fp, '// This file was generated by \'' + PG.ThisWorkbook.Name + '\'  ' + X02.Time, '\n')
    VBFiles.writeText(fp, '', '\n')
    if Correct_Pins:
