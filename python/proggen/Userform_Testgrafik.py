...
class UserForm_TestGrafik:
        ...
        self.Main_Menu_Form_RSC = {"UserForm":{
                        ...
                        "Components": [{"Name": "TopFrame","BackColor": "#00000F","BorderColor": "#000006","BorderStyle": "fmBorderStyleNone",
                                      ...
                                      {"Name": "Frame2","BackColor": "#00000F","BorderColor": "#000006","BorderStyle": "fmBorderStyleNone",
                                       "Caption"       : "Bild",
                                       "ControlTipText": "","ForeColor": "#000012","Height": 3000,"Left": 1,"Top": 200,"Type": "Frame","Visible": True,"Width": 5000, "Geo_Manager" : "grid",
                                       "Components"    :
                                            [
                                                ]},
                                      {"Name": "LowerFrame","BackColor": "#00000F","BorderColor": "#000006","BorderStyle": "fmBorderStyleNone",
                                       ...
                                       "Components"    :
                                        ...
                                        {"Name":"OK_Button","Accelerator":"<Return>","BackColor":"#00000F","BorderColor":"#000006","BorderStyle":"fmBorderStyleNone",
                                         "Caption":"OK",
                                         "Command":self.OK_Button_Click,"ControlTipText":"","ForeColor":"#000012","Height":2,"Left":1,"Top":1,"Type":"CommandButton","Visible":True,"Width":10},
                                        {"Name":"Abort_Button","Accelerator":"<Escape>","BackColor":"#00000F","BorderColor":"#000006","BorderStyle":"fmBorderStyleNone",
                                         "Caption":"Abbrechen",
                                         "Command":self.Abort_Button_Click,"ControlTipText":"","ForeColor":"#000012","Height":2,"Left":2,"Top":1,"Type":"CommandButton","Visible":True,"Width":10},
                                        {"Name":"Update_Button","Accelerator":"u","BackColor":"#00000F","BorderColor":"#000006","BorderStyle":"fmBorderStyleNone",
                                         "Caption":"Update Grafik",
                                         "Command":"","ControlTipText":"","ForeColor":"#000012","Height":2,"Left":3,"Top":1,"Type":"CommandButton","Visible":True,"Width":10},
                                        ]
                                      }
                                ]}
                        }
    ...
    def __UserForm_Initialize(self):
        ...
        self.Bck_Image_TextBox.Value = self.get_value_from_current_sheet(2,1)
        ...
        on_off_line = 0 # self.calculate_on_off_line()
    ...
    def Show_With_Existing_Data(self, MacroName):
        Txt = String()
        #----------------------------------------------------------------------------------------------------------------------
        self.Macro_str = ""
        self.Show()
    ...
    def get_value_from_current_sheet(self, row, col):
        current_sheet = XLA.ActiveSheet
        value = current_sheet.get_cellvalue_direct(current_sheet, row, col)
        return value
    
    def save_value_to_current_sheet(self, row, col, newval):
        current_sheet = XLA.ActiveSheet
        current_sheet.set_value_in_cell(row, col, newval)
        return
        
    def calculate_complete_curve(self):
        max_row = XLA.ActiveSheet.getRowCount()
        stdpos_x = 255
        stdpos_y = 10
        stdpos_delta = 10
        curidx = 0
        self.curve_points = []
        for index in range(3, max_row):
            # if row active
            if self.get_value_from_current_sheet(index,2) != "":
                pos_value = self.get_value_from_current_sheet(index,1)
                if pos_value != "":
                    pos_value_split = pos_value.split(",")
                    stdpos_y = int(pos_value_split[1])
                    stdpos_x = int(pos_value_split[0])
                else:
                    stdpos_y += stdpos_delta
                    stdpos_x = 255
                description = self.get_value_from_current_sheet(index,7)
                macro = self.get_value_from_current_sheet(index,12)
                tooltip = "Zeile:" + str(index) + "\n" + description + "\n" + macro
                self.curve_points.append([stdpos_x, stdpos_y, index, tooltip])
                stdpos_y += stdpos_delta
    ...
    def Select_Button_Click(self, event=None):
        filename = tk.filedialog.askopenfilename()
        self.Bck_Image_TextBox.Value = filename
        return    
    
    def OK_Button_Click(self, event=None):
        for cur_point in self.curve_points:
            xpos = cur_point[0]
            ypos = cur_point[1]
            row = cur_point[2]
            col = 1
            newval = str(xpos) + "," + str(ypos)
    ...
    def init_graph(self, frame=None, vertical_params=None, horizontal_params=None, num_vertical=None, num_horizontal=None):
        ...
        self.canvasframe = CF.CanvasFrame(frame, self.frameheight, self.framewidth, self.Background_Image)
        self.canvasframe.grid(row=0, column=0) #,sticky="nesw")
    ...
    def draw_point(self, x, y, xvalue, yvalue, index, midx=None, tooltip=None):
        objid = -1
        if True: #self.Servo_CurveType_ListBox.Selection == curvetype_individuell:
            python_green = "#00FF00"
            size = self.pointsize = 8
            x1, y1 = (x - size), (y - size)
            x2, y2 = (x + size), (y + size)
            objid = self.canvas.create_oval(x1, y1, x2, y2, fill=python_green, tag=["point", "x:" + str(xvalue), "y:" + str(yvalue), "idx:" + str(index)], width=2, activewidth=4)
            if tooltip != None:
                Tooltip_Canvas(self.canvas, objid, text=tooltip,button_1=True)
            ...
            self.canvas.tag_bind(objid,"<Double-Button-1>" ,lambda e,Object=objid:self.MouseDoubleButton1(e,Object))
    ...
    def draw_graph(self, line_params=None,  vertical_params=None, horizontal_params=None, num_vertical=8, num_horizontal=10, graph=None, vertical_range=(0, 256), horizontal_range=(0, 20000), on_off_line = 0):
        ...
        #try:
        #    BackgroundPathName =  self.Bck_Image_TextBox.Value
        #    self.background_id = Image.open(BackgroundPathName)
        #    self.background = ImageTk.PhotoImage(self.background_id)
        #    self.canvas.create_image(0, 0, anchor='nw', image=self.background)
        #    self.bg_width, self.bg_height = self.background_id.size
        #    self.imscale = 1.0  # scale for the canvaas image
        #    self.delta = 1.3  # zoom magnitude            
        #except BaseException as e:
        #    print(e)
        ...
        if graph != []:
            ...
            for i in range(len(graph)):
                ...
                timepoint = graph[i][0]
                cur_y = graph[i][1] #int((value * self.vertical_value_factor))
                cur_x = graph[i][0] #int(graph[i][0] * self.horizontal_value_factor)
                self.graph_points.extend([self.tx2cx(cur_x), self.ty2cy(cur_y)])
                cur_midx =  graph[i][2]
                cur_tooltip =   graph[i][3]
                # draw_vertical line
                ...
                #self.canvas.tag_bind(objid,"<Button-1>" ,lambda e,Object=objid:self.MouseButton1(e,Object))
                #self.canvas.tag_bind(objid,"<Double-Button-1>" ,lambda e,Object=objid:self.DoubleMouseButton1(e,Object))
                #self.canvas.tag_bind(objid,"<ButtonRelease 1>",lambda e,Object=objid:self.MouseRelease1())
                ...
                #    last_text_x = cur_x
                point_id = self.draw_point(self.tx2cx(cur_x), self.ty2cy(cur_y), timepoint, value, i, midx=cur_midx, tooltip=cur_tooltip)
        ...
        if cvobject != -1: #and self.Servo_CurveType_ListBox.Selection == 2: # individuel
            ...
            if objecttype == "point":
                ...
                if point_idx_str[0] == "idx":
                    ...
                    row = self.curve_points[self.point_idx][2] 
                    col = 4
                    curr_sheet = XLA.ActiveSheet
    ...
    def MouseDoubleButton1(self,event,cvobject):
        self.startxy = None
        self.canvas.focus_set()
        self.point_idx = -1
        ...
        if cvobject != -1: #and self.Servo_CurveType_ListBox.Selection == 2: # individuel
            ...
            if objecttype == "point":
                ...
                if point_idx_str[0] == "idx":
                    ...
                    target.Select()
                    cancel = None
                    curr_sheet.Evt_SheetBeforeDoubleClick(curr_sheet,target, cancel)
    ...
    def MouseMove(self,event,cvobject):
        ...
        x1, y1 = (x - size), (y - size)
        x2, y2 = (x + size), (y + size)
        self.canvas.coords(cvobject, x1, y1, x2, y2)
        if self.point_idx != - 1:
            self.mm_canvas_coord_list[self.point_idx*2+1] = y
            self.mm_canvas_coord_list[self.point_idx*2] = x
            self.current_curve_point = [x, y]
            #print("MouseMove:", val_x, val_y, self.current_curve_point, pos_x, pos_y, event.x, event.y)
            self.curve_points[self.point_idx][0] = x
            self.curve_points[self.point_idx][1] = y
