...
class Sheet(tk.Frame):
    ...
    def highlight(
        ...
        *key: CreateSpanTypes,
        ...
        span = self.span_from_key(*key)
        if bg is False and fg is False and font is False and end is None:
            return span
        rows, cols = self.ranges_from_span(span)
        table, index, header = span.table, span.index, span.header
        if span.kind == "cell":
            if header:
                for c in cols:
                    add_highlight(self.CH.cell_options, c, bg, fg, font, end, overwrite)
            for r in rows:
                if index:
                    add_highlight(self.RI.cell_options, r, bg, fg, font, end, overwrite)
                if table:
                    for c in cols:
                        add_highlight(self.MT.cell_options, (r, c), bg, fg, font, end, overwrite)
        elif span.kind == "row":
            for r in rows:
                if index:
                    add_highlight(self.RI.cell_options, r, bg, fg, font, end, overwrite)
                if table:
                    add_highlight(self.MT.row_options, r, bg, fg, font, end, overwrite)
        elif span.kind == "column":
            for c in cols:
                if header:
                    add_highlight(self.CH.cell_options, c, bg, fg, font, end, overwrite)
                if table:
                    add_highlight(self.MT.col_options, c, bg, fg, font, end, overwrite)
        self.set_refresh_timer(redraw)
        return span
    ...
    def hide_rows(
        ...
        if not _rows:
            return
        if self.MT.all_rows_displayed:
            _d_rows = [r for r in range(self.MT.total_data_rows()) if r not in _rows]
        else:
            _d_rows = [r for r in self.MT.displayed_rows if r not in _rows] #[e for r, e in enumerate(self.MT.displayed_rows) if r not in _rows]
        self.display_rows(
            rows=_d_rows,
    ...
    def highlight_rows(
        ...
        if bg is None and fg is None and font is None:
            return
        for r in (rows,) if isinstance(rows, int) else rows:
            add_highlight(self.MT.row_options, r, bg, fg, font, end_of_screen, overwrite)
        if highlight_index:
            self.highlight_cells(cells=rows, canvas="index", bg=bg, fg=fg, font=font, redraw=False)
        self.set_refresh_timer(redraw)
    ...
    def highlight_columns(
        ...
        if bg is False and fg is False and font is False:
            return
        for c in (columns,) if isinstance(columns, int) else columns:
            add_highlight(self.MT.col_options, c, bg, fg, font, None, overwrite)
        if highlight_header:
            self.highlight_cells(cells=columns, canvas="header", bg=bg, fg=fg, font=font, redraw=False)
        self.set_refresh_timer(redraw)
    ...
    def highlight_cells(
        ...
        if canvas == "table":
            if cells:
                for r_, c_ in cells:
                    add_highlight(self.MT.cell_options, (r_, c_), bg, fg, font, None, overwrite)
            else:
            ...
            for r_ in iterable:
                add_highlight(self.RI.cell_options, r_, bg, fg, font, None, overwrite)
        elif canvas == "header":
            iterable = (
                cells if (cells and not isinstance(cells, int)) else (cells,) if isinstance(cells, int) else (column,)
            )
            for c_ in iterable:
                add_highlight(self.CH.cell_options, c_, bg, fg, font, None, overwrite)
        self.set_refresh_timer(redraw)
