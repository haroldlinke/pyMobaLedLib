...
class MainTable(tk.Canvas):
    ...
    def enable_bindings_internal(self, binding):
        ...
        if binding in ("all", "undo", "edit_bindings"):
            self.undo_enabled = False #test True
    ...
    def b1_release(self, event=None):
        if self.being_drawn_item != None: #is not None:
            currently_selected = self.currently_selected()
    ...
    def redraw_highlight_get_text_fg(
        ...
        tfont = None
        if kwargs:
            tfont = self.PAR.ops.table_font if kwargs[3] is None else kwargs[3]
        ...
        if redraw_table:
            ...
            for c in range(start_col, end_col - 1):
                for r in rows_:
                    ...
                    fill, font, dd_drawn = self.redraw_highlight_get_text_fg(
                        r,
                        c,
                        cleftgridln,
                        rtopgridln,
    ...
    #**** test cellfont *****
    def get_cell_font(self, r: int, c: int) -> str:
        datarn = self.datarn(r)
        datacn = self.datacn(c)
        cell_alignment = self.get_cell_kwargs(datarn, datacn, key="font")
        if cell_alignment:
            return cell_alignment
        return self.align    
    #**** test cellfont *****
    ...
    def get_cell_data(
        ...
        none_to_empty_str: bool = True, #HL
    ...
    def datarn(self, r: int) -> int:
        return r if (self.all_rows_displayed or r >= len(self.displayed_rows)) else self.displayed_rows[r]
    ...
    def ToolTip_canvas(self, objid,text="",button_1=False):
        if text != "":
            tooltiptext = text
            tooltip_var = self.tooltip_var_dict.get(objid,None)
            if tooltip_var==None:
                tooltip_var=Tooltip_Canvas(self,objid,text=tooltiptext,button_1=False,controller=self)
