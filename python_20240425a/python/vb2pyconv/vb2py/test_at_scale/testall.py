...
if __name__ == "__main__":
    ...
    #
    files = glob.glob(r"test*.py")
    files.sort()
    preferentialSort(files, ['testtonamalu.py'])
    #
    total_run = 0
    total_failed = 0
    start = time.time()
    try:
        for file in files:
            if file not in (r"testall.py", r"testframework.py", "testparser.py"):
                fname = os.path.join((r"python %s" % vb2py.utils.rootPath()), 'test_at_scale', file)
