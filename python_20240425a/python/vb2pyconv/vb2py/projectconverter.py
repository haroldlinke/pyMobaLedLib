...
class VBConverter(object):
    ...
    def doConversion(self, filename, destinationdir, callback=None):
        ...
        project = self.parser(filename,destinationdir)
        project.doParse()
        ...
        for module in project.modules:
            ...
            mod = ModuleParser(os.path.join(project_root, module_filename.strip()), destinationdir, module_name)
        ...
        for form in project.forms:
            ...
            frm = FormParser(os.path.join(project_root, form),destinationdir)
        ...
        for cls in project.classes:
            ...
            class_mod = ClassParser(os.path.join(project_root, cls_filename.strip()), destinationdir, cls_name)
...
class BaseParser(object):
    ...
    def __init__(self, filename, destinationdir, name=None):
        """Initialize the parser"""
        self.code_structure = None
        self.references = []
        self.filename = filename
        filename_split = os.path.splitext(os.path.split(filename)[1])
        self.name = name or filename_split[0]
        self.ext = filename_split[1]
    ...
    def readFileContent(filename):
        ...
        #
        vb_code = vb_code.replace("Else:","Else\n")
        vb_code = vb_code.replace("Then Call","Then")
        vb_code = vb_code.replace(" ' ","\n' ")
        return vb_code
    ...
    def _getPattern(id_pattern):
        ...
        return re.compile(r'^%s\s*=\s*"*(.*?)"*$' % id_pattern, re.MULTILINE + re.UNICODE)
...
class ProjectParser(BaseParser):
    ...
    def excelparse(self,workbook_path):
        vba_path = vba_path = workbook_path + '.vba' # 'src.vba'
        vba_parser = VBA_Parser(workbook_path)
        vba_modules = vba_parser.extract_all_macros() if vba_parser.detect_vba_macros() else []
        
        #HLtest From generation
        form_parser = VBA_Parser(workbook_path)
        res=form_parser.extract_form_strings_extended()
        for filename, stream_path, form_string in res:
            print(filename,stream_path,form_string)
        ...
        self.classes = []
        self.references = []
        self.overwritefiles = Config["General", "OverwriteFiles"]== "Yes"
            ...
            if lines:
                ...
                if content and content[-1] == '':
                    ...
                    if non_empty_lines_of_code > 0:
                        ...
                        if not os.path.isfile(os.path.join(filename+".bas")) or self.overwritefiles: # only extract macro when macro is not already available
                            with open(os.path.join(filename+".bas"), 'w', encoding='utf-8') as f:
                                f.write('\n'.join(content))
                                
                        fname = os.path.join(self.destinationdir, filename) + ".py"
                        if not os.path.isfile(fname) or self.overwritefiles: # only parse when python file is not already available
                            self.modules.append(filename+";"+filename+".bas")
            

    def doParse(self):
        """Parse the text"""
        if self.filename.endswith(EXCEL_FILE_EXTENSIONS):
            self.excelparse(self.filename)
        else:
...
class FormParser(BaseParser):
    ...
    def parseForm(self):
        ...
        #
        # End
        pattern = re.compile(r"^\s*End$", re.MULTILINE + re.UNICODE)
        self.form_data = pattern.sub("", self.form_data)

        #
        # End Property
        pattern = re.compile(r"^\s*EndProperty$", re.MULTILINE + re.UNICODE)
        self.form_data = pattern.sub("", self.form_data)

        #
        # Version
        pattern = re.compile(r"^VERSION\s+.*?$", re.MULTILINE + re.UNICODE)
        self.form_data = pattern.sub("", self.form_data)
        ...
        pattern = re.compile(r'\$?"(.*)\.frx":(\S+)', re.MULTILINE + re.UNICODE)
        ...
        pattern = re.compile(r"\&H([A-F0-9]{8})\&", re.MULTILINE + re.UNICODE)
...
def main():
    ...
    conv.doConversion(project_file,destination_dir)
...
def vb2py():
    ...
    os.chdir(path)    
    if not os.path.isdir(destination_dir): # destination dir is a subdir of path
        print("Second argument must be a valid directory")
        sys.exit(2)
