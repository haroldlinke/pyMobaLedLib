...
class VBObject(VBNamespace):
    ...
    def replacefunctionname(self,fct_name):
        parent=self.parent
        found=False
        _fn_return_value=fct_name
        while not found and parent!=None:
            if hasattr(parent,"block_name"):
                if parent.block_name=="fn_block":
                    _fn_return_value= parent.name_substitution.get(fct_name,fct_name)
                    found = True
            if not found:
    ...
    def _renderPartialObject(self, indent=0, modifier=None):
        ...
        #
        #HL no nameextension in with statement
        if implicit_name == "":
...
class VBProject(VBNamespace):
    ...
    def __init__(self, scope="Private"):
        ...
        self.import_replacements = Config.getItemNames("ImportReplacements")
        self.import_extension = Config["ImportReplacements", "StdImportExtension"]
        
    def refine_modulename(self,modulename):
        
        if modulename.lower() in self.import_replacements:
            import_replacement = Config["ImportReplacements", modulename.lower()]
            log.info("Processing module replacement %s (%s)" % (modulename, import_replacement))
            modulename = import_replacement.split(",")[0]
        else:
            if self.import_extension != "":
                modulename = self.import_extension + "." + modulename
            log.info("Processing module replacement %s (%s)" % (modulename, self.import_extension))
    ...
    def resolveLocalName(self, name, rendering_locals=0, requestedby=None):
        ...
        if name in self.global_objects:
            ...
            modulename = self.refine_modulename(modulename)
...
class VBModule(VBCodeBlock):
    ...
    def refine_import(self,item):
        
        if item.lower() in self.import_replacements:
            import_replacement = Config["ImportReplacements", item.lower()]
            log.info("Processing module replacement %s (%s)" % (item, import_replacement))
            item = import_replacement.split(",")[1]
        else:
            import_extension = Config["ImportReplacements", "StdImportExtension"]
            if import_extension != "":
                item = import_extension + "." + item
        return item            

    def importStatements(self, indent=0):
        """Render the standard import statements for this block"""
        
        other = [""] + ["import %s" % self.refine_import(item) for item in self.module_imports]  # Leading [""] gives a newline
    ...
    def resolveLocalName(self, name, rendering_locals=0, requestedby=None):
        ...
        for obj in self.locals:
            try:
                ...
                print("ResolveLocalName: Error accessing obj.identifier")
        raise UnresolvableName("Name '%s' is not known in this namespace" % name)
    ...
    def setCustomModulesAsGlobals(self):
        ...
        for module_id in custom_modules:
            ...
            vbmodule = VBCodeModule(modulename="%s" % module_name)
            #
